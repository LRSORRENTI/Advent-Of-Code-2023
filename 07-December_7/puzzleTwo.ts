...
// Custom Type for Card Strength
type CardStrength = '2' | '3' | '4' | '5' | '6' | '7' | '8' | '9' | 'T' | 'Q' | 'K' | 'A' | 'J';

// Custom Interface for Hand
interface Hand {
    hand: string;
    bid: number;
}
...
function getType(hand: string): number {
    ...
    const countJokers: number = hand.split("").filter(e => e === 'J').length;
    ...
    const eachCardsCount: number[] = Object.values(cardsCount).sort((a, b) => b - a);
    const uniqueCardsCount: number = eachCardsCount.length;
